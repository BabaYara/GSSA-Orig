		     ...
		     int penalty, int normalize, REAL* B)
{
  ...
  if(RM == 1){
    ...
    if(typeid(realtype) == typeid(singletype)){
      sgels('N', T, n1, N, (float*)X1, Tplus1, (float*)Y1, T, &info);
    } else if(typeid(realtype) == typeid(doubletype)){
      dgels('N', T, n1, N, (double*)X1, Tplus1, (double*)Y1, T, &info);
    }
    for(ix = 0 ; ix < n1 ; ++ix){
      for(jx = 0 ; jx < N ; ++jx){
  ...
  //==========================================================================
    ...
    // all the regressions, j=1,...,N, are run at once
    if(typeid(realtype) == typeid(singletype)){
      sgelss(T, n1, N, (float*)X1, Tplus1, (float*)Y1, T, (float*)S, f_rcond, &rank, &info);
    } else if(typeid(realtype) == typeid(doubletype)){
      dgelss(T, n1, N, (double*)X1, Tplus1, (double*)Y1, T, (double*)S, d_rcond, &rank, &info);
    }
    for(ix = 0 ; ix < n1 ; ++ix){
      for(jx = 0 ; jx < N ; ++jx){
	B1[ix+jx*n1] = Y1[ix+jx*T];
