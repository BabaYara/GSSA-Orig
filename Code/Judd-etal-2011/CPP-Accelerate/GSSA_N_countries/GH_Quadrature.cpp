		   ...
		   REAL* weight_nodes)
{
  ...
  if(typeid(realtype) == typeid(singletype)){
    float f_alpha = 1.0;
    float f_beta = 0.0;
    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_nodes, N, N, f_alpha, (float*)z, n_nodes,
	   (float*)vcv, N, f_beta, (float*)epsi_nodes, n_nodes);
  } else if(typeid(realtype) == typeid(doubletype)){
    double d_alpha = 1.0;
    double d_beta = 0.0;
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_nodes, N, N, d_alpha, (double*)z, n_nodes,
