...
int main()
{
  ...
  while(dif_1d > 1e-4*kdamp){
	...
	cblas_sgemv(CblasColMajor, CblasTrans, nStates, N, f_alpha,
		    (float*)&bk_1d[0], nStates, (float*)&x[0]+ix, inc_x,
		    f_beta, (float*)&k[0]+(ix+1), inc_k);
      } else if(typeid(realtype) == typeid(doubletype)){
	cblas_dgemv(CblasColMajor, CblasTrans, nStates, N, d_alpha,
		    (double*)&bk_1d[0], nStates, (double*)&x[0]+ix, inc_x,
		    d_beta, (double*)&k[0]+(ix+1), inc_k);
      }
    }
    ...
    lwork = -1;
    if(typeid(realtype) == typeid(singletype)){
      sgels_(&notrans, &Tminus1, &nStates, &N, (float*)&x[0], &T,
	     (float*)&Y_copy[0], &Tminus1, &f_wkopt, &lwork, &info);
      lwork = (int)f_wkopt;
    } else if(typeid(realtype) == typeid(doubletype)){
      dgels_(&notrans, &Tminus1, &nStates, &N, (double*)&x[0], &T,
	     (double*)&Y_copy[0], &Tminus1, &d_wkopt, &lwork, &info);
      lwork = (int)d_wkopt;
    }
    work.resize(lwork);
    if(typeid(realtype) == typeid(singletype)){
      sgels_(&notrans, &Tminus1, &nStates, &N, (float*)&x[0], &T,
	     (float*)&Y_copy[0], &Tminus1, (float*)&work[0], &lwork, &info);
    } else if(typeid(realtype) == typeid(doubletype)){
      dgels_(&notrans, &Tminus1, &nStates, &N, (double*)&x[0], &T,
	     (double*)&Y_copy[0], &Tminus1, (double*)&work[0], &lwork, &info);
    }
  ...
  for(D = 1 ; D <= D_max ; ++D){
	...
	if(typeid(realtype) == typeid(singletype)){
	  cblas_sgemv(CblasColMajor, CblasTrans, n_cols, N, f_alpha,
		      (float*)&bk_D[0], n_cols, (float*)&poly_X_row[0],
		      inc_row, f_beta, (float*)&k_row[0], inc_row);
	} else if(typeid(realtype) == typeid(doubletype)){
	  cblas_dgemv(CblasColMajor, CblasTrans, n_cols, N, d_alpha,
		      (double*)&bk_D[0], n_cols, (double*)&poly_X_row[0],
		      inc_row, d_beta, (double*)&k_row[0], inc_row);
	}
	for(jx = 0 ; jx < N ; ++jx){
	...
	for(hx = 0 ; hx < n_nodes ; ++hx){
	  ...
	  Ord_Polynomial_N(&X1_integral[0], T, 2*N, D, &poly_X_integral[0]);

	  // Compute capital of period t+2 (chosen at t+1) using the
	  // capital policy functions; n_nodes-by-N 
	  if(typeid(realtype) == typeid(singletype)){
	    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, T, N,
			n_cols, f_alpha, (float*)&poly_X_integral[0], T,
			(float*)&bk_D[0], n_cols, f_beta, (float*)&k2[0], T);
	  } else if(typeid(realtype) == typeid(doubletype)){
	    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, T, N,
			n_cols, d_alpha, (double*)&poly_X_integral[0], T,
			(double*)&bk_D[0], n_cols, d_beta, (double*)&k2[0], T);
	  }
  ...
  for(D = 1 ; D <= D_max ; ++D){
	...
	X1_row_test[jx] = X1_test[ix+jx*T_test];
      ...
      if(typeid(realtype) == typeid(singletype)){
	cblas_sgemv(CblasColMajor, CblasTrans, n_cols, N, f_alpha,
		    (float*)&bk[0], n_cols, (float*)&poly_X_row_test[0],
		    inc_row, f_beta, (float*)&k_row_test[0], inc_row);
      } else if(typeid(realtype) == typeid(doubletype)){
	cblas_dgemv(CblasColMajor, CblasTrans, n_cols, N, d_alpha,
		    (double*)&bk[0], n_cols, (double*)&poly_X_row_test[0],
		    inc_row, d_beta, (double*)&k_row_test[0], inc_row);
      }
      for(jx = 0 ; jx < N ; ++jx){
