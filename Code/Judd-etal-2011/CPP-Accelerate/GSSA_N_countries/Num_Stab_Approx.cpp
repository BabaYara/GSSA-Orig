		     ...
		     int penalty, int normalize, REAL* B)
{
  ...
  if(RM == 1){
    ...
    if(typeid(realtype) == typeid(singletype)){
      sgels_(&notrans, &T, &n1, &N, (float*)X1, &Tplus1, (float*)Y1, &T, &f_wkopt, &lwork, &info);
      lwork = (int)f_wkopt;
    } else if(typeid(realtype) == typeid(doubletype)){
      dgels_(&notrans, &T, &n1, &N, (double*)X1, &Tplus1, (double*)Y1, &T, &d_wkopt, &lwork, &info);
      lwork = (int)d_wkopt;
    }
    work = (REAL*)realloc(work, lwork*sizeof(REAL));
    if(typeid(realtype) == typeid(singletype)){
      sgels_(&notrans, &T, &n1, &N, (float*)X1, &Tplus1, (float*)Y1, &T, (float*)work, &lwork, &info);
    } else if(typeid(realtype) == typeid(doubletype)){
      dgels_(&notrans, &T, &n1, &N, (double*)X1, &Tplus1, (double*)Y1, &T, (double*)work, &lwork, &info);
    }
    for(ix = 0 ; ix < n1 ; ++ix){
      for(jx = 0 ; jx < N ; ++jx){
  ...
  //==========================================================================
    ...
    lwork = -1;
    if(typeid(realtype) == typeid(singletype)){
      sgelss_(&T, &n1, &N, (float*)X1, &Tplus1, (float*)Y1, &T, (float*)S, &f_rcond, &rank, &f_wkopt, &lwork, &info);
      lwork = (int)f_wkopt;
    } else if(typeid(realtype) == typeid(doubletype)){
      dgelss_(&T, &n1, &N, (double*)X1, &Tplus1, (double*)Y1, &T, (double*)S, &d_rcond, &rank, &d_wkopt, &lwork, &info);
      lwork = (int)d_wkopt;
    }
    work = (REAL*)realloc(work, lwork*sizeof(REAL));
    if(typeid(realtype) == typeid(singletype)){
      sgelss_(&T, &n1, &N, (float*)X1, &Tplus1, (float*)Y1, &T, (float*)S, &f_rcond, &rank, (float*)work, &lwork, &info);
    } else if(typeid(realtype) == typeid(doubletype)){
      dgelss_(&T, &n1, &N, (double*)X1, &Tplus1, (double*)Y1, &T, (double*)S, &d_rcond, &rank, (double*)work, &lwork, &info);
    }
    for(ix = 0 ; ix < n1 ; ++ix){
      for(jx = 0 ; jx < N ; ++jx){
	B1[ix+jx*n1] = Y1[ix+jx*T];
  ...
  */
  ...
  //==========================================================================
    ...
    if(typeid(realtype) == typeid(singletype)){
      float f_alpha = 1.0;
      float f_beta = 0.0;
      cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, n1, n1, T, f_alpha, (float*)X1, Tplus1, (float*)X1, Tplus1, f_beta, (float*)XX, n1);
      cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, n1, N, T, f_alpha, (float*)X1, Tplus1, (float*)Y1, T, f_beta, (float*)XY, n1);
    } else if(typeid(realtype) == typeid(doubletype)){
      double d_alpha = 1.0;
      double d_beta = 0.0;
      cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, n1, n1, T, d_alpha, (double*)X1, Tplus1, (double*)X1, Tplus1, d_beta, (double*)XX, n1);
      cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, n1, N, T, d_alpha, (double*)X1, Tplus1, (double*)Y1, T, d_beta, (double*)XY, n1);
    }
    ...
    int info;
    if(typeid(realtype) == typeid(singletype)){
      sgesv_(&n1, &N, (float*)XX, &n1, ipiv, (float*)XY, &n1, &info);
    } else if(typeid(realtype) == typeid(doubletype)){
      dgesv_(&n1, &N, (double*)XX, &n1, ipiv, (double*)XY, &n1, &info);
    }
    for(ix = 0 ; ix < n1 ; ++ix){
      for(jx = 0 ; jx < N ; ++jx){
