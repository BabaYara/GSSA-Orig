		 ...
		 REAL* weight_nodes)
{
  ...
  if(typeid(realtype) == typeid(singletype)){
    spotrf_(&uplo, &N, (float*)vcv, &N, &info);
  } else if(typeid(realtype) == typeid(doubletype)){
    dpotrf_(&uplo, &N, (double*)vcv, &N, &info);
  }
  for(ix = 1 ; ix < N ; ++ix){
    for(jx = 0 ; jx < ix ; ++jx){
      vcv[ix + jx*N] = 0.0;
  ...
  if(typeid(realtype) == typeid(singletype)){
    ...
    float f_beta = 0.0;
    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nrows_z1, N, N,
		f_alpha1, (float*)z1, nrows_z1, (float*)vcv, N, f_beta,
		(float*)z1_alt, nrows_z1);
    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nrows_z2, N, N,
		f_alpha2, (float*)z2, nrows_z2, (float*)vcv, N, f_beta,
		(float*)z2_alt, nrows_z2);
  } else if(typeid(realtype) == typeid(doubletype)){
    double d_alpha1 = sqrt(N+2);
    double d_alpha2 = sqrt((N+2)/2);
    double d_beta = 0.0;
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nrows_z1, N, N,
		d_alpha1, (double*)z1, nrows_z1, (double*)vcv, N, d_beta,
		(double*)z1_alt, nrows_z1);
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nrows_z2, N, N,
		d_alpha2, (double*)z2, nrows_z2, (double*)vcv, N, d_beta,
		(double*)z2_alt, nrows_z2);
  }
