		 ...
		 REAL* weight_nodes)
{
  ...
  if(typeid(realtype) == typeid(singletype)){
    ...
    float f_beta = 0.0;
    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_nodes, N, N, 
		f_alpha, (float*)z1, n_nodes, (float*)vcv, N, f_beta,
		(float*)epsi_nodes, n_nodes);
  } else if(typeid(realtype) == typeid(doubletype)){
    double d_alpha = sqrt(N);
    double d_beta = 0.0;
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_nodes, N, N,
		d_alpha, (double*)z1, n_nodes, (double*)vcv, N, d_beta,
		(double*)epsi_nodes, n_nodes);
  }
